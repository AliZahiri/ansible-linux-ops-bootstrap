...
class DailyPrTests(unittest.TestCase):
    ...
    def test_branch_name_uses_daily_prefix(self):
        self.assertEqual(daily_pr.branch_name({"id": "inventory-validation"}), "daily/inventory-validation")
    ...
    def test_load_tasks_rejects_duplicate_task_ids(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tasks = [
                sample_task(),
                {**sample_task(), "target_file": "docs/other.md"},
            ]
    ...
    def test_load_tasks_compile_checks_python_without_executing_it(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(
                ValueError,
                "task sample has invalid Python in scripts/broken.py",
            ):
